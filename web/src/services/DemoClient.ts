...
export class DemoClient {
  ...
  private investigate(id: string, start: number, o: { related: number; conflicts: number; edits: string[]; tests: number; files: number; ins: number; del: number }): Step[] {
    ...
    return [
      ...
      { at: t0 + 1300, run: () => { this.step(id, "Booted a sandbox from my desk's snapshot", { layer: 'daytona' }); this.agent(DEMO_USER, { working: [{ cardId: id, sandboxId: `sb-${id.slice(-6)}`, step: 'forking' }] }) } },
      ...
      { at: t0 + 1550, run: () => this.patch(id, (c) => ({ ...c, evidence: { ...c.evidence, timeline: (c.evidence?.timeline || []).map((s) => s.label.startsWith('Booted') ? { ...s, status: 'done' as const, detail: '5.4 s · checkout and dependencies already there' } : s) } })) },
      ...
      { at: t0 + 6400, run: () => this.finish(id, (ev) => ({
        ...
        dryRun: {
          sandboxId: 'sb-' + id.slice(-6), deskId: `sb-desk-${DEMO_USER}`, forked: true, branch: `agent/card-${id.slice(-8)}`, bootMs: 5400,
